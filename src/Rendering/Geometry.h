 ...
 */
    struct Geometry {
    public:
        vVertex vertices; /**< Util of vertex data. */
        vIndex indices; /**< Util of index data. */
         ...
         * @param vertices Util of vertex data.
         * @param indices Util of index data.
 ...
 */
    ...
    struct TextureQuad {
        int height, width, colorChannels, size;
        GLubyte *textureData;
        vVertex vertices;

        TextureQuad(int height, int width, int colorChannels);
        ~TextureQuad();

        void updateVertices(vVertex vertices);
